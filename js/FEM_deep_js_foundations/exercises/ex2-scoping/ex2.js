function initUI() {
  // these are assignments, declared at the bottom
...
function validateWorkEntry(description,minutes) {
  // TODO - the hardcoded numbers could be constants
...
function addProject(description) {
  ...
  // projectId only exists in the couple lines of code, it isn't needed for
  // the push or as a param to the other functions, so it can be a let. 
  { let projectId;
	    projectId = Math.round(Math.random()*1E4);
	    projectEntryData = { id: projectId,
